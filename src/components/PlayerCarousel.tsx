...
const Carousel: React.FC = () => {
  ...
  // Resize → only update cardsPerPage
  useEffect(() => {
    const updateCardsPerPage = () => {
      const width = window.innerWidth;
      if (width >= 1024) setCardsPerPage(4);       // lg
      else if (width >= 768) setCardsPerPage(1);   // md
      else setCardsPerPage(1);                     // sm
    };
    updateCardsPerPage();
  ...
  const handleScroll = () => {
    ...
    const section = Math.round((scrollLeft / (totalScroll || 1)) * Math.max(totalPages - 1, 0));
  ...
  const scrollToIndex = (index: number) => {
    ...
    track.scrollTo({ left: index * cardWidth * cardsPerPage, behavior: 'smooth' });
  };

  const scrollLeftHandler = () => scrollToIndex(Math.max(activeIndex - 1, 0));
  const scrollRightHandler = () => scrollToIndex(Math.min(activeIndex + 1, Math.max(totalPages - 1, 0)));
    ...
    <div className="carousel-container select-none relative">
      ...
      {isLoading ? (
        <div
          role="status"
          aria-busy="true"
          className="flex items-center justify-center h-64 md:h-[60vh] bg-white/50 text-gray-700 font-medium"
        >
          <Loading />
        </div>
      ) : cards.length === 0 ? (
        <div className="flex items-center justify-center h-64 md:h-[60vh] text-gray-600">
          No players found.
        </div>
      ) : (
        <>
          ...
          <button
            ...
            disabled={isLoading}
            className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10 bg-white p-2 rounded-full shadow text-black cursor-pointer hover:text-white hover:bg-red-600 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <FaArrowLeft />
          </button>
          ...
          <button
            ...
            disabled={isLoading}
            className="absolute right-2 top-1/2 transform -translate-y-1/2 z-10 bg-white p-2 rounded-full shadow text-black cursor-pointer hover:text-white hover:bg-red-600 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <FaArrowRight />
          </button>
          ...
          <div className="carousel-track md:h-[80vh] lg:h-[auto]" ref={trackRef}>
            {cards.map((item, i) => (
              <div className="card text-black" key={item.id ?? i}>
                <motion.div
